...
Read `styles.css` and its import `_ds_bundle.css` for the exact classes and tokens, and `components/<group>/<Name>/<Name>.prompt.md` + `<Name>.d.ts` for each component's props and story JSX. Groups: `controls` (Button, Fader, Keybed, Knob, Segmented, Select, Stepper, Toggle, Wheel), `layout` (Panel, SectionHeader, Tabs), `display` (Meter, ValueReadout, WavetableDisplay).
...
- `Wheel` — the pitch or mod wheel of a performance strip: `value` 0..1, `onChange`, required `label`, `defaultValue` (rest position, default 0), `bipolar` (the fill grows from `defaultValue` instead of from the bottom), `springBack` (returns to `defaultValue` when the pointer is released — a pitch wheel), `tone`, `disabled`. **It carries no height of its own**: the body is `flex-1` inside a column, so the parent must be a flex container with a definite height — `<div className="flex h-fader"><Wheel …/></div>`. In a plain block parent only the label renders.
- `Keybed` — a playable keyboard: `firstNote` (MIDI note of the first white key, 36 = C2), `octaves` (default 4), `velocity` 0..1 for pointer-played notes, `onNoteOn(note, velocity)`, `onNoteOff(note)`, `onAllNotesOff`. Optional `subscribeNotes(cb) => unsubscribe` lights keys from the engine: the callback receives a `KeybedNoteMask` and the component mutates `data-active` by ref instead of re-rendering — never drive lit keys through a value prop. It fills its parent (`h-full w-full`), so give the parent both a height and a width (`<div className="h-32 w-full">`).
...
- Helper types referenced by the props but not declared in the `.d.ts` files: `Tone = "osc" | "filter" | "env" | "lfo" | "fx" | "master"`, `KnobMod = { tone: Tone; depth: number; bipolar?: boolean }`, `TabItem = { value: string; label: string; tone?: Tone }`, `SegmentedOption = { value: string; label: string }`, `KeybedNoteMask = readonly [number, number, number, number]` (one bit per MIDI note, word `note >> 5`, bit `note & 31`).
