...
- Restyling hooks (no props): every drawn part of `Knob` carries `data-part` (`track`, `value`, `cap-edge`, `cap`, `rim`, `grip`, `pointer`, `pointer-groove`, `pointer-tip`, `label`, `readout`) and each `Keybed` key carries `data-key="white" | "black"` plus `data-active`. A consuming app changes a control's material (glass, brushed metal) with CSS on these attributes and on `data-slot`; the `data-testid` attributes are for tests only and are not a styling contract.
