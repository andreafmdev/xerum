...
export function OscPanel() {
  ...
  const pickWt = (delta: number) => wt.set(wt.options[step(wtIndex, delta, wt.options.length)]!.value);
  return (
    <Panel
      title="Oscillator"
      tone="osc"
      ...
      actions={
        <>
          <Segmented label="Unison voices" value={unison.value} onChange={unison.set} options={unison.options} />
      ...
      <div className={body}>
        <div className="flex h-full w-32 shrink-0 flex-col justify-between gap-1.5 self-stretch">
          ...
          <div className="flex flex-col gap-1.5">
            <Stepper value={oct.value} onChange={oct.set} min={oct.min} max={oct.max} unit="OCT" label="Octave" format={signedInt} />
            <Stepper value={semi.value} onChange={semi.set} min={semi.min} max={semi.max} unit="SEMI" label="Semitones" format={signedInt} />
          </div>
        </div>
...
export function FilterPanel() {
  ...
  const { mods, sources } = useSynthCtx();
  const W = 236;
  const H = 58;
  // Il grafico segue la cutoff modulata, non solo quella impostata.
  const cutLive = liveValue(cutoff.value, modsFor(mods, "cutoff"), sources);
  const d = useMemo(() => filterPath(cutLive, res.value, ftype.value as FilterType, W, H), [cutLive, res.value, ftype.value]);
  return (
    <Panel
      title="Filter"
      tone="filter"
      ...
      actions={
        <>
          ...
          <Segmented label="Slope" value={slope.value} onChange={slope.set} options={slope.options} />
        </>
      }
    >
  ...
  return (
    <Panel title="Master" tone="master" className={`${plate} flex-[0_0_176px]`}>
      <div className="px-2.5">
        <Segmented label="Voice mode" value={voiceMode.value} onChange={voiceMode.set} options={voiceMode.options} />
      </div>
      <div className={`${body} justify-around`}>
