...
**3. Generalizzare `ParameterSeamTests` a un ciclo su `kTable`.** Oggi copre 8 parametri scelti a mano su 48. Un ciclo che verifichi, per ogni spec, che `getRawParameterValue` al default valga il default dichiarato **avrebbe fatto fallire la build il primo giorno**. Prerequisito: **generare la mappatura id→slot**, oggi scritta a mano in tre posti (`ParamSlot.h`, `PluginProcessor.cpp`, e il `RealAccessor` del test stesso — cioè il test verifica il proprio cablaggio). **[TECNICA]**
