...
**Headroom.** `Source/engine/SynthVoice.cpp` applies a fixed per-voice gain, `kVoiceHeadroomGain = 0.71f` (-3 dB), to every sample before panning — not a `1 / numVoices` divisor, which would pump the overall level up and down every time a note starts or ends. Measured at the *plugin* output with `level = 1.0` and `volume = 1.0`, filter at Butterworth Q: a single note sits at **-15.4 dBFS RMS**, -8.7 dBFS peak. At the default `volume` of 0.8 the same note peaks at -8.8 dBFS and a four-note chord at -0.9 dBFS; those two come from `Tests/EngineTests.cpp`, "gain staging: una nota e un accordo normale stanno sotto il soft clipper", which pins them below the output soft clipper's threshold and above an audibility floor.

That constant is not a round number picked for comfort: **the four-note chord sets it.** At `level = 1.0` and the default `volume`, four notes present 0.900 to the soft clipper against its 0.95 threshold. One decibel more per voice and the safety net would fire during ordinary playing, which is the one thing it must not do. The reference level everybody quotes for this kind of instrument — around -14 dBFS RMS on a single note, where Serum and Vital sit — is therefore *not reachable here*, and the reason is arithmetic rather than caution: the crest factor is fixed at 6.7 dB between RMS and peak on one note, and summing four notes adds another 9.7 dB of peak, so a four-note chord always lands 16.4 dB above a single note's RMS. Putting that RMS at -14 dBFS would put the chord at +2.4 dBFS. The two targets do not coexist; the clean chord wins, and the instrument settles 1.4 dB below the reference.

The constant used to be `0.4f` (-8 dB), which left that single note at -20.4 dBFS RMS — an instrument you had to lift by six decibels in the mixer before you could judge it. Before that it was `0.1f` (-20 dB), chosen when two other stages could each add uncontrolled gain: the filter multiplied its resonance peak stage by stage (see below), and `saturate()` had a small-signal slope of 1.5 so it added +3.5 dB even at drive zero.

**What the retune cost.** Every stage downstream of the voice scales with this constant, the extreme cases included, and the resonance compensation below only claws back part of it. Before and after, all at the soft clipper's input:

| | -8 dB / `1/32` | -3 dB / `1/8` |
|---|---|---|
| single note, `level` 1, `volume` 1 | -20.4 dBFS RMS | **-15.4 dBFS RMS** |
| four-note chord, default `volume` | 0.506 (-5.9 dBFS) | **0.900 (-0.9 dBFS)** |
| 16 held notes | 1.19 (+1.5 dBFS) | 2.11 (+6.5 dBFS) |
| full matrix, `res` modulated to the top | 2.52 (+8.0 dBFS) | 3.48 (+10.8 dBFS) |
| the same with unison 8 | 2.59 (+8.3 dBFS) | 3.52 (+10.9 dBFS) |
| passband loss at Q 24 | -0.64 dB | -3.51 dB |
| resonant peak at Q 24, over Butterworth | +29.7 dB | +26.8 dB |

The extreme cases got **louder, not quieter**, by about 2.7 dB. Holding them still while the instrument gained five decibels would have needed the resonance compensation to take back the whole five, i.e. an exponent near 0.29, which costs 8.9 dB of passband — twice the budget. There is no setting of these two constants that raises ordinary playing and leaves the extremes where they are; the compensation buys 2.2 dB of the five, and the rest lands on the clipper. It is still 3 dB short of the point (+14 dBFS) where the clipper would stop being a net and become a distortion stage on its own, but the margin that existed before is mostly spent.
...
*The gain compensation is `1 / sqrt(N)`.* Summing N uncorrelated oscillators grows as the root, not linearly; without it, going from unison 1 to 8 would add about 9 dB. Measured: −15.36 dBFS RMS at unison 1 against −16.33 at unison 8.
...
It used to be `1/32` (−0.64 dB at Q 24), and before that `1/2`, which was simply wrong: **−14.7 dB of passband at Q 24**, so turning resonance up made the whole instrument quieter and thinner instead of making the filter ring — measured on the engine at −9.99 dB of output RMS at Q 24, against +4.37 dB after that change. Going from `1/32` to `1/8` is the opposite move, made on purpose and with the eyes open: it costs 2.9 dB of passband at Q 24 and takes the same 2.9 dB off the resonant peak, which drops from +29.7 dB to **+26.8 dB** over Butterworth (2.2 dB of it at Q 12, the top of `res`'s own travel). Those are the decibels that pay for part of the headroom rise above — the only lever in the instrument that touches the extremes harder than it touches ordinary playing.

That peak is still the instrument's loudest source of transient level: the output soft clipper below is what keeps it in bounds. Since `res` became a modulation target this is no longer a corner case reachable only by turning a knob — a route can drive it there every LFO cycle, and the full-matrix stress test (see **Modulation**) measures about +10.8 dBFS arriving at the clipper because of it.
...
The threshold was 0.8 and moved up with the headroom — for the opposite reason to the obvious one. It is not that less signal arrives now; more does, and at 0.8 an ordinary four-note chord (0.900 at the clipper's input) would have engaged it, turning the net into a permanent distortion stage. At 0.95 the chord passes untouched with half a decibel to spare, and so do all twelve factory presets: the hottest of them, *Acid Line*, reaches 0.84 on four notes. The price is a knee that shrinks from 0.2 to 0.05 of travel, so when the clipper does fire — 16 held notes present 2.11, the full matrix 3.48 — it bends much more abruptly than before. That is the right trade only because those cases are the exception, and because a higher threshold also leaves a wider slice of the waveform bit-identical: everything between 0.8 and 0.95 is now untouched.
...
It holds, and the margin is now thinner than it was: the worst case measures **−0.01 dBFS at the output, which is 3.48, or +10.8 dBFS, arriving at the soft clipper** (it was 2.52, +8.0 dBFS, before the gain staging was retuned; unison 8 adds another tenth of a decibel). With `res` modulated to the top of its travel that is the filter's resonant peak — the +26.8 dB over Butterworth described above — landing on a sixteen-note chord. The soft clipper is the only thing between that and truncated samples, so the test asserts that it *does* engage (peak above the 0.95 threshold, which under a clipper that is transparent below it is a proof that the input was above it too): a stress test that quietly stopped exercising the stage it exists to protect would still pass, and would be worth nothing.

**The remaining headroom is 3 dB, and it is worth watching.** The instrument gained 5 dB per voice and the resonance compensation gave back 2.2 of them, so this case moved from +8.0 to +10.8 dBFS. Past roughly +14 dBFS the clipper stops behaving like a net — a deliberately absurd patch (16 voices, unison 8, 100 cents of detune, Q 6, +12 dB of drive) already measures +14.7 dBFS. Anything that raises per-voice level again, or that adds a gain stage after the filter, has to come with a matching reduction here; the two constants above are out of slack.
...
**Every preset is tuned against the gain staging**, which is why each one spells out `level`, `drive` and `volume` explicitly instead of inheriting them: those three decide whether it clips. When `kVoiceHeadroomGain` went from -8 dB to -3 dB all twelve got 5 dB louder, and one of them, *Reese Wide*, stopped fitting: a four-note chord took it to 1.108 at the soft clipper's input, past full scale. Its `level` went from 0.8 to 0.6, which puts it at 0.831 — in line with the rest of the bass presets rather than 2.5 dB above them. The other eleven needed nothing: the loudest of them, *Acid Line*, reaches 0.842 on four notes, and the quietest, *Dust Choir*, 0.225. Anything above 0.95 would engage the clipper, so that is the line a retuned or new preset has to stay under.
